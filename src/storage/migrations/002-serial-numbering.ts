...
export function applyMigration002(db: Database.Database): void {
  // ALTER TABLE must be outside the transaction (SQLite DDL forces implicit commit)
  ...
  db.transaction(() => {
    db.exec(`
      ...
      CREATE INDEX IF NOT EXISTS idx_prefix_reservations_expires ON prefix_reservations(expires_at);
    ...
    db.exec(`
      ...
      CREATE INDEX IF NOT EXISTS idx_note_metadata_series ON note_metadata(series) WHERE series IS NOT NULL;
      CREATE INDEX IF NOT EXISTS idx_note_metadata_project_id ON note_metadata(project_id) WHERE project_id IS NOT NULL;
    `);
